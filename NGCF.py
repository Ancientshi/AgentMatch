...
# -*- coding: utf-8 -*-
"""
simple_ngcf_agent_rec.py
NGCF for Query (question) ↔ Agent implicit feedback with BPR loss.
+ TF-IDF KNN cold-start for questions at eval/predict time (graph_scope=train).

Example:
  python simple_ngcf_agent_rec.py \
      --data_root /path/to/dataset_root \
      --epochs 10 --batch_size 4096 --emb_dim 128 --layers 3 \
      --neg_per_pos 1 --reg 1e-4 --topk 10 --device cuda:0 \
      --init_from_tfidf 1 --tfidf_fit_scope train \
      --graph_scope train --knn_N 8 --eval_cand_size 1000
      
      
没问题！我把 NGCF 版也按你 LightGCN 的逻辑加上了 TF-IDF KNN 冷启动（eval/predict 阶段）。核心思路一致：

训练图只用 graph_scope=train 的边；

训练完后用 传播后的 U_train（NGCF 输出的用户向量）+ 训练问题文本 TF-IDF 建立 KNN 缓存；

评估/预测时，若某个 qid 在训练图中 无边（冷启动），就用 TF-IDF 找到 K 个最相似训练问题的 U_train 做 相似度加权均值 作为该问题的用户向量，再与所有 item 打分。

直接替换下面这份脚本即可（搜索 # === KNN cold-start additions === 查看新增/改动处）：
"""
...
def collect_data(data_root: str):
    """
    Reads PartI|II|III + Tools and merges them.
    """
...
class NGCF(nn.Module):
    """
    NGCF with per-layer W1, W2, LeakyReLU, and layer-wise mean aggregation.
    h^{k+1} = LeakyReLU( (A_hat @ h^k) W1_k + (A_hat @ (h^k ⊙ h^k)) W2_k )
    A_hat = D^{-1/2} A D^{-1/2} + I
    """
    def __init__(self, num_users: int, num_items: int, emb_dim: int, n_layers: int,
                 norm_adj: torch.sparse.FloatTensor, reg: float = 1e-4,
                 device="cpu", agg: str = "mean", negative_slope: float = 0.2):
        super().__init__()
        assert agg in ("mean",), "This script uses 'mean' aggregation for simplicity."
        ...
        # Base normalized adjacency (U+I) x (U+I) + I (self-loops)
        self.norm_adj = norm_adj.coalesce().to(device)
        N = num_users + num_items
        eye_idx = torch.arange(N, device=device, dtype=torch.long)
        eye_indices = torch.stack([eye_idx, eye_idx], dim=0)
        eye_values = torch.ones(N, device=device, dtype=torch.float32)
        I_sparse = torch.sparse_coo_tensor(eye_indices, eye_values, (N, N)).coalesce()
        self.A_hat = (self.norm_adj + I_sparse).coalesce()
    ...
    def propagate(self):
        x = torch.cat([self.user_emb.weight, self.item_emb.weight], dim=0)  # (U+I, D)
        xs = [x]
        for l in range(self.n_layers):
            neigh  = torch.sparse.mm(self.A_hat, x)
            neighg = torch.sparse.mm(self.A_hat, x * x)
            x = self.act(self.W1[l](neigh) + self.W2[l](neighg))
            xs.append(x)
        out = torch.stack(xs, dim=0).mean(0)  # mean over layers (incl. layer-0)
    ...
    def score(self, u_idx: torch.Tensor, i_idx: torch.Tensor):
        ...
        return (U[u_idx] * I[i_idx]).sum(dim=1)

    def bpr_loss(self, u_idx: torch.Tensor, pos_idx: torch.Tensor, neg_idx: torch.Tensor):
        U_all, I_all = self.get_user_item()
        u = U_all[u_idx]; pos = I_all[pos_idx]; neg = I_all[neg_idx]
        pos_scores = (u * pos).sum(dim=1); neg_scores = (u * neg).sum(dim=1)
...
def build_norm_adj(num_users: int, num_items: int, user_pos: List[Tuple[int, int]]):
    ...
    for (u, i) in user_pos:
        ui = U + i
        rows.extend([u, ui]); cols.extend([ui, u])
    if not rows:
...
@torch.no_grad()
def evaluate_sampled_with_knn(model: NGCF,
    ...
    for qid in pbar:
        ...
        if user_has_edge[qi]:
            ...
            u_np = _knn_user_vec_for_text(qtext, knn_cache_path, N=knn_N)
...
def main():
    ...
    # TF-IDF init options
    parser.add_argument("--init_from_tfidf", type=int, default=0, help="1: init user/item embeddings from TF-IDF + SVD")
    ...
    parser.add_argument("--tfidf_fit_scope", type=str, default="train", choices=["train","all"],
                        help="fit query vectorizer on train-only or all queries (for SVD init)")

    # === KNN cold-start args ===
    parser.add_argument("--knn_N", type=int, default=4, help="TF-IDF KNN neighbors for cold queries")
    ...
    # Model
    device = torch.device(args.device)
    model = NGCF(num_users, num_items, args.emb_dim, args.layers, norm_adj,
                 reg=args.reg, device=device, agg='mean').to(device)
    ...
    if args.init_from_tfidf == 1:
        ...
        Q_basis = q_vec.transform([get_query_text(all_questions[qid]) for qid in q_ids]).astype(np.float32)
        ...
        U0 = svd_to_dim(Q_basis, args.emb_dim)
        V0 = svd_to_dim(A_basis, args.emb_dim)
        with torch.no_grad():
    ...
    # === Build & save KNN cache from TRAIN questions (using propagated U) ===
    ...
    # Demo: recommend for a few questions (with KNN fallback)
    @torch.no_grad()
