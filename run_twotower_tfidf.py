...
def evaluate_sampled_twotower(
    ...
    def _binary_hits_soft(pred_ids: List[str], rel_llm_set: set[str], k: int) -> List[int]:
        """Count a relevant LLM at most once within top-k (first occurrence wins)."""
        hits = []
        seen = set()
    ...
    for qid in pbar:
        ...
        if need_neg > 0:
            attempts = 0
            # small cand_size -> rejection sampling is fine and avoids building huge neg_pool
        ...
        if not soft_eval:
            ...
            # LLM-level relevance; each relevant LLM counted at most once in top-k
...
def main() -> None:
    ...
    if feature_cache_exists(feature_cache_dir) and args.rebuild_feature_cache == 0:
        ...
        if vecs is None:
            feature_cache, vecs = build_twotower_feature_cache(
    ...
    # agent_id -> backbone LLM string (from feature cache)
    ...
    for i, aid in enumerate(a_ids):
        ...
        llm = llm_vocab[idx] if 0 <= idx < len(llm_vocab) else ""
        aid_to_llm[aid] = llm
    ...
    def build_cache():
        qids_train_pool = [q for q in qids_in_rank if qid_to_part.get(q, "") in set(args.train_parts)]
    ...
    qids_eval = [q for q in qids_in_rank if qid_to_part.get(q, "") in set(args.eval_parts)]
    print(f"[eval] #qids_eval={len(qids_eval)} (parts={args.eval_parts})")
    ...
    part_splits = split_eval_qids_by_part(qids_eval, qid_to_part=qid_to_part)

    for part in args.eval_parts:
