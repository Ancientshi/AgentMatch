...
# -*- coding: utf-8 -*-
"""
simple_simgcl_agent_rec_tfidf.py
LightGCN for Query ↔ Agent (implicit) with BPR loss,
**TF-IDF-initialized** embeddings + **SimGCL** self-supervised contrastive loss,
and **KNN-based cold-start** for questions at eval/pred time (graph_scope=train).

Example:
  python simple_simgcl_agent_rec_tfidf.py \
      ...
      --ssl_lambda 0.1 --ssl_temp 0.2 --ssl_noise 0.1 --ssl_users 8192 --ssl_items 8192
"""
...
class LightGCN(nn.Module):
    ...
    # --- Cached variant (避免重复 propagate；供 SimGCL 训练循环使用) ---
    def bpr_loss_cached(self, U_all, I_all, u_idx: torch.Tensor, pos_idx: torch.Tensor, neg_idx: torch.Tensor):
        u = U_all[u_idx]
        pos = I_all[pos_idx]
        neg = I_all[neg_idx]
        pos_scores = (u * pos).sum(dim=1)
...
# ---------------- SimGCL helpers ----------------
def _unit_noise_like(x: torch.Tensor) -> torch.Tensor:
    n = torch.randn_like(x)
    return F.normalize(n, dim=1)

def _perturb(x: torch.Tensor, eps: float) -> torch.Tensor:
    return F.normalize(x + eps * _unit_noise_like(x), dim=1)

def nt_xent(z1: torch.Tensor, z2: torch.Tensor, temp: float) -> torch.Tensor:
    z1 = F.normalize(z1, dim=1)
    z2 = F.normalize(z2, dim=1)
    sim = (z1 @ z2.t()) / temp            # (N, N)
    pos = sim.diag()                       # (N,)
    loss = - (pos - torch.logsumexp(sim, dim=1))
    return loss.mean()

def simgcl_ssl_loss(U_all: torch.Tensor, I_all: torch.Tensor,
                    user_sample: int, item_sample: int,
                    eps: float, temp: float, device: torch.device) -> torch.Tensor:
    Nu = U_all.size(0)
    Ni = I_all.size(0)
    if user_sample > 0 and Nu > 0:
        idx_u = torch.randperm(Nu, device=device)[:min(user_sample, Nu)]
        u = U_all[idx_u]
        zu1 = _perturb(u, eps)
        zu2 = _perturb(u, eps)
        lu = nt_xent(zu1, zu2, temp)
    else:
        lu = U_all.new_tensor(0.0)

    if item_sample > 0 and Ni > 0:
        idx_i = torch.randperm(Ni, device=device)[:min(item_sample, Ni)]
        v = I_all[idx_i]
        zv1 = _perturb(v, eps)
        zv2 = _perturb(v, eps)
        li = nt_xent(zv1, zv2, temp)
    else:
        li = I_all.new_tensor(0.0)
...
# ---------- Evaluation (sampled) with KNN cold-start ----------
...
def main():
    ...
    # ----- SimGCL / contrastive learning options -----
    parser.add_argument("--ssl_lambda", type=float, default=0.0,   help="weight for SimGCL loss (0=off)")
    parser.add_argument("--ssl_temp",   type=float, default=0.2,   help="NT-Xent temperature")
    parser.add_argument("--ssl_noise",  type=float, default=0.1,   help="perturbation scale epsilon")
    parser.add_argument("--ssl_users",  type=int,   default=8192,  help="num users sampled per step for SSL (<= #users)")
    parser.add_argument("--ssl_items",  type=int,   default=8192,  help="num items sampled per step for SSL (<= #items)")
    ...
    # -------- Feature cache (queries & agents' text features) --------
    ...
    # -------- Dataset signature & model paths --------
    data_sig = dataset_signature(a_ids, all_rankings)
    model_path = os.path.join(cache_dir, "models", f"{filename}_{data_sig}.pt")
    latest_model = os.path.join(cache_dir, "models", f"latest_{data_sig}.pt")
    meta_path = os.path.join(cache_dir, "models", f"meta_{data_sig}.json")
    ...
    want_meta = {
        ...
        "ssl_temp": float(args.ssl_temp),
        "ssl_noise": float(args.ssl_noise),
        "ssl_users": int(args.ssl_users),
        "ssl_items": int(args.ssl_items),
    }

    # -------- Train/valid split & training pairs (cacheable) --------
    ...
    # mask: which user nodes have at least one train edge (for KNN cold-start at eval)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for b in pbar:
            ...
            # 一次传播，得到全图嵌入（用户/物品） -> 避免每次在 bpr_loss 内重复 propagate
            U_all, I_all = model.get_user_item()

            # BPR（使用缓存嵌入）
            loss_bpr = model.bpr_loss_cached(U_all, I_all, u_idx, p_idx, n_idx)

            # SimGCL SSL（可选）
            if args.ssl_lambda > 0.0:
                loss_ssl = simgcl_ssl_loss(
                    U_all, I_all,
                    user_sample=args.ssl_users,
                    item_sample=args.ssl_items,
                    eps=args.ssl_noise,
                    temp=args.ssl_temp,
                    device=device
                )
                loss = loss_bpr + args.ssl_lambda * loss_ssl
            else:
            ...
            pbar.set_postfix({
                ...
                "avg_loss": f"{(total/(b+1)):.4f}"
            })

        print(f"Epoch {epoch}/{args.epochs} - loss: {(total/num_batches if num_batches>0 else 0.0):.4f}")
    ...
    print_metrics_table("Validation (SimGCL + KNN cold-start; averaged over questions)", valid_metrics, ks=(5,10,50), filename=filename)
